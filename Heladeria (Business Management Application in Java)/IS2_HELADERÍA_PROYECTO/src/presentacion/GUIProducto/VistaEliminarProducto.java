...
private void crearOyentes(){
	aceptar.addActionListener(e->{
		...
		producto.setId(idP);
		Controlador.getInstancia().accion(Eventos.ELIMINAR_PRODUCTO,producto);
	});
	cancelar.addActionListener(e->{this.dispose();});
}
public void actualizar(int evento, Object datos){
	   if (evento == Eventos.RES_ELIMINAR_PRODUCTO_OK) {
		  this.dispose();
		 JOptionPane.showMessageDialog(this, "Se ha eliminado el producto con id "+(Integer)datos);
		 }
		else if(evento == Eventos.RES_ELIMINAR_PRODUCTO_KO) {
		 this.dispose();
		 JOptionPane.showMessageDialog(this, "No se ha podido eliminar el producto indicado");
		 }
}
}