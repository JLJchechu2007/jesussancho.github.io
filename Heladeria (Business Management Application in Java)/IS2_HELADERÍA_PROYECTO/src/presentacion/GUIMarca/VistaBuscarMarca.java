...
public class VistaBuscarMarca extends JFrame implements IGUI{
	...
	private void crearoyentes() {
		aceptar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				VistaBuscarMarca.super.setVisible(false);
				VistaBuscarMarca.this.dispose();
				int _id=Integer.parseInt( id.getText());
				TMarca tmarca= new TMarca();
				tmarca.setId(_id);
				Controlador.getInstancia().accion(Eventos.BUSCAR_MARCA, tmarca);
			}
		});
	...
	public void actualizar(int evento, Object datos) {
		if(evento==Eventos.RES_BUSCAR_MARCA_OK) {
			TMarca tmarca=(TMarca)datos;
			JOptionPane.showMessageDialog(this, "Nombre de la marca: "+ tmarca.getNombre()+ "\n" +"Nombre del jefe de la marca: "+tmarca.getNombreJefe()+ "\n"+ "Lugar de la sede de la marca: "+tmarca.getLugarSede()+"\n");
			this.dispose();
		}
		else if(evento==Eventos.RES_BUSCAR_MARCA_KO) {
			this.dispose();
			JOptionPane.showMessageDialog(this, "No se ha encontrado la marca");
		}
